 ...
 * @param {boolean} [options.unsafePerm] (optional) Whether unsafe permissions should be used for npm scripts. Default: false
 * @param {string} [options.cwd] (optional) The directory to execute the command in
 * @param {NodeJS.ReadStream}  [options.stdin]  (optional) Where to redirect the stdin. Default: process.stdin
 * @param {NodeJS.WriteStream | "buffer"} [options.stdout] (optional) Where to redirect the stdout or "buffer" to return it as a string. Default: process.stdout
 * @param {NodeJS.WriteStream | "buffer"} [options.stderr] (optional) Where to redirect the stderr or "buffer" to return it as a string. Default: process.stderr
 * @returns {Promise<NpmCommandResult>}
 */
function npmCommandAsync(command, npmArgs, options, callback) {
    if (typeof npmArgs === 'function') {
        // npmCommand(command, callback);
        callback = npmArgs;
        npmArgs = undefined;
        options = undefined;
    } else if (typeof npmArgs === 'object' && !Array.isArray(npmArgs)) {
        // npmCommand(command, options, callback?);
    ...
    if (typeof options === 'function') {
        // npmCommand(command, npmArgs?, callback);
        callback = options;
        options = undefined;
    }
