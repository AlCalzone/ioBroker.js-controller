 ...
 * @property {boolean} unsafePerm Whether unsafe permissions should be used for npm scripts. Default: false
 * @property {boolean} logCommandExecution Whether the executed command should be logged to the stdout. Default: true
...
function npmCommand(command, npmArgs, options) {
    ...
    if (options.logCommandExecution == null) options.logCommandExecution = true;
    if (options.logCommandExecution) {
        console.log(
            'system command: '
            + `${npmBinary} ${[command].concat(npmArgs).join(" ")}`
        );
    }
