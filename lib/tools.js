...
/**
 * Locates the root directory of JS-Controller
 * @returns {string}
 */
function getControllerDir() {
    return path.join(__dirname, '..'); // one level up from /lib/
}
const controllerDir = getControllerDir();

/**
 * Locates the installation root directory that contains everything related to ioBroker
 * Returns nothing if the root dir was not found
 * @returns {string | void}
 */
function getAppRootDir() {
    // Step 1: Find the directory where JS-controller is located
    const controllerDir = getControllerDir();
    // Step 2: Find out if this package was installed via npm
    const npmParentDir = path.join(controllerDir, '../..'); // the directory where node_modules is potentially located
    const installedWithNpm = fs.existsSync(path.join(controllerDir, npmParentDir, `node_modules/${getAppName()}.js-controller`));
    // If this was installed with npm, the folder containing node_modules is the root directory
    if (installedWithNpm) return npmParentDir;
}
 ...
 */
function disablePackageLock() {
    if (packageLockDisabled) return Promise.resolve();
 ...
 * @property {boolean} [unsafePerm] (optional) Whether unsafe permissions should be used for npm scripts. Default: false
 * @property {string} [cwd] (optional) The directory to execute the command in
 * @property {NodeJS.ReadStream}  [stdin]  (optional) Where to redirect the stdin. Default: process.stdin
 * @property {NodeJS.WriteStream | "pipe" | "ignore"} [stdout] (optional) A write stream to redirect the stdout, "ignore" to ignore it or "pipe" to return it as a string. Default: process.stdout
 * @property {NodeJS.WriteStream | "pipe" | "ignore"} [stderr] (optional) A write stream to redirect the stderr, "ignore" to ignore it or "pipe" to return it as a string. Default: process.stderr
 */
 ...
 * @param {NpmCommandOptions} [options] (optional) Some options for the command execution
...
function npmCommand(command, npmArgs, options) {
    ...
    if (options.unsafePerm && npmArgs.indexOf('--unsafe-perm') === -1) npmArgs.push('--unsafe-perm');
 ...
 * @param {NpmCommandOptions} [options] (optional) Some options for the command execution
 * @param {boolean} [debug=false] (optional) Whether to log debug output
 * @returns {Promise<void>}
 */
function npmUninstall(packageName, options, debug) {

    // Find the root directory to uninstall from
    const installDir = getAppRootDir() || getControllerDir();
    options.cwd = installDir;
    // In debug mode redirect the stdout of npm, otherwise ignore it
    options.stdout = debug ? process.stdout : 'ignore';
    // stderr is always passed through
    options.stderr = process.stdout;

    const npmArgs = [packageName, '--silent', '--save'];

    return disablePackageLock()
        .then(() => npmCommand("uninstall", npmArgs, options))
        .then(result => {
            // code 1 is strange error that cannot be explained. Everything is installed but error :(
            if (result.exitCode !== 0 && result.exitCode !== 1) {
                return Promise.reject(`host.${getHostName()}: Cannot uninstall ${packageName}: ${result.exitCode}`);
            } else {
                return Promise.resolve();
            }
        });
 ...
 * @param {string} npmUrl The package source to install from
 * @param {NpmCommandOptions} [options] (optional) Some options for the command execution
 * @param {boolean} [debug=false] (optional) Whether to log debug output
 * @returns {Promise<{npmUrl: string, node_modules_dir: string}>}
 */
function npmInstall(npmUrl, options, debug) {

    // Find the root directory to install into
    const installDir = getAppRootDir() || getControllerDir();
    options.cwd = installDir;
    // In debug mode redirect the stdout of npm, otherwise ignore it
    options.stdout = debug ? process.stdout : 'ignore';
    ...
    const npmArgs = [npmUrl, '--production', '--save'];

    return disablePackageLock()
        .then(() => npmCommand("install", npmArgs, options))
        .then(result => {
            // code 1 is strange error that cannot be explained. Everything is installed but error :(
            if (result.exitCode !== 0 && result.exitCode !== 1) {
                return Promise.reject(`host.${getHostName()}: Cannot install ${npmUrl}: ${result.exitCode}`);
            } else {
                // TODO: is this actually necessary???
                // create file that indicates, that npm was called there
                if (npmUrl.indexOf(':') === -1) {
                    const packageDir = path.join(installDir, 'node_modules', npmUrl);
                    if (fs.existsSync(packageDir)) {
                        fs.writeFileSync(path.join(packageDir, 'iob_npm.done'), ' ', 'utf8');
                    }
                }
                return Promise.resolve({
                    npmUrl,
                    node_modules_dir: path.join(installDir, 'node_modules')
                });
            }
        });
};
